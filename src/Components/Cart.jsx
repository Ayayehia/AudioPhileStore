...
dispatch(RemoveAll())
    ...
    <Modal onClick={funcOpen}>
        ...
        {Data.map((prod)=><CartProducts data={prod} style={"quantityContainer"} styleTwo={"container"}/>)}
    <h1 className={Classes.totalAmount}><span style={{opacity: "0.5"}}>Total</span><span>{totalPrice}$</span> </h1>
    <Link to="/checkout"><button className={Classes.btn}> Check Out</button></Link>
    </Modal >
  )
}
